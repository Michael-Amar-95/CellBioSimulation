    ...
    def meiosis(self):
        ...
        if self.num_copies % 2 == 0:
            ...
            cell_origin = Cell(self.name, self.genome, int(self.num_copies / 2)
                               , self.genetic_code, self.start_codons, cell.error_rate, self.division_rate)
...
class MutantCell(StemCell):
    ...
    def __init__(self, genome, num_mutations=0):
        ...
        if num_mutations > 10:
            super().__init__('CancerCell', genome, 0.05, 10)
        else:
            super().__init__('MutantCell', genome, 0.05, 3)
...
"""This function help me to find the max num of cells"""


def mitosis_cycles(cells_list, max_cells, max_cycles):
    number_of_cells = 1
    ...
    while current_division_cycles < max_cycles:
        ...
        while i < cell_list_length:
            ...
            if number_of_cells + (cells_list[i].division_rate - 1) <= max_cells:
                temp_cells_list += cells_list[i].mitosis()
                number_of_cells += cells_list[i].division_rate - 1
            elif number_of_cells + (
                    cells_list[0].division_rate - 1) <= MaxCells and current_division_cycles + 2 <= max_cycles:
                current_division_cycles += 1
                temp_cells_list += (cells_list[0].mitosis())[1:]
                number_of_cells += cells_list[0].division_rate - 1
                i -= 1
            else:
                # help me to know if i got to this scope
                flag = False
                temp_cells_list += cells_list[i:]
                break
            i += 1
        current_division_cycles += 1
...
if __name__ == '__main__':
    ...
    assert len(sys.argv) == 2, 'need to be one argument'
    path = sys.argv[1]

    # try to read from the file
    try:
        with open(os.path.join(os.path.dirname(__file__), path), 'r') as input_file:
            loader_data = input_file.read()
    except:
        raise AssertionError(" can't open the file ")

    # check if i got a JSON file
    try:
        a_json = json.loads(loader_data)
    except:
        raise AssertionError("String could not be converted to JSON")

    path_genome = a_json.get('GenomicSequencesInput')
    # try to read from the file
    try:
        with open(os.path.join(os.path.dirname(__file__), path_genome), 'r') as genome_file:
            genomes_str = genome_file.read()
    except:
        raise AssertionError(" can't open the file ")

    # upper the chars
    genomes_str = genomes_str.upper()

    # separate the genomes
    genome_list = genomes_str.split('\n')

    # check if the nucleotide are good
    char_set = {'A', 'T', 'G', 'C'}
    for i in genome_list:
        dna_set = set(i)
        assert dna_set.issubset(char_set), str(dna_set)

    mutantCell = MutantCell(genome_list)

    # check if MaxCycles exist in json file
    try:
        MaxCycles = a_json.get('MaxCycles')
    except:
        raise AssertionError('no field MaxCycles in json file')
    # check max cycles
    assert int(MaxCycles) > 0, 'num fo division need to be a positive integer'

    # check if MaxCells exist in json file
    try:
        MaxCells = a_json.get('MaxCells')
    except:
        raise AssertionError('no field MaxCells in json file')
    # check max cell
    assert int(MaxCells) > 1, 'num of max cells need to be a positive integer'

    cells_list = [mutantCell]
    cells_list = mitosis_cycles(cells_list, MaxCells, MaxCycles)
    # print all what we ask for
    print('Original cell: ' + mutantCell.__repr__())
    print('Final number of cells: ' + str(len(cells_list)))
    data = []
    protein_list = []
    ...
    sequenceClassifier = SequenceClassifier(a_json.get('PatternsToDomainsInput'))
    # delete duplicate protein
    protein_list = list(dict.fromkeys(protein_list))
    sequenceClassifier.classify(protein_list, a_json.get('OutputFile'))
    cells_genome_list = []
    for i in cells_list:
        for j in i.genome:
            cells_genome_list.append(j)

    # printing
    print('Protein repertoire size: ' + str(len(protein_list)))
    print('Mutations: ' + str(max_mutation(cells_list)))
